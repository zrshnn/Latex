...
\subsubsection{Reearch Locale}

\begin{figure}[H]
	\centering
	\caption{Map of Camiguin}
	\label{fig:map}
	\includegraphics[width=0.5\textwidth]{figures/map.png}
\end{figure}

The research will be conducted in specific area of Camiguin, Philippines that is known for frequent and unscheduled power interruptions. This area is served by CAMELCO (Camiguin Electric Cooperative), which experience regular power outages, particularly during peak hours. This issue disrupts daily activities, especially in community centers, educational institutions, and local businesses that rely on electricity for mobile devices, lighting, and appliances.

\subsection{System Design}
This section outlines the system design. It 
...
This section outlines the criteria and specifications for the materials and components used in the final device prototype. Each component was chosen based on specific criteria to ensure ---

The following table outlines the key considerations for choosing hardware components for a system. It details the specific features, specifications, and desired functionalities for each component.
...
\end{longtable}
	...
	& 
	\vspace{0.1cm}
	The researchers selected the ESP32 microcontroller as the central processing unit of the system. Its built-in Wi-Fi and Bluetooth modules, low power consumption, and compact size make it suitable for IoT-based applications. The ESP32 efficiently manages all sensors, relays, and peripheral devices, providing reliable performance for the rental power station. \\ \hline	
	...
	\hline
		...
		\vspace{0.1cm} % adds space above image
		\includegraphics[width=0.8\linewidth]{figures/router.png}
...
\subsubsection{Schematic Layout}
\begin{figure}[H]
	\centering
...
This part provides an outline of the software development for the solar-powered rental station with detachable power sources. The primary objective of the software is to manage the rental, monitoring, and control of portable power units through data acquisition, processing, and decision-making. The system integrates embedded system programming, mobile application development, and cloud-based data management to ensure reliable, automated, and secure operation.

The software is designed to monitor battery status, track rental duration, determine location coordinates, and calculate user credits based on predefined system rules. These functionalities allow the system to support automated unlocking and locking mechanisms, notification handling, and synchronization between hardware, mobile application, and cloud services.

\subsubsection{Embedded System Software Development}
...
The unused power is used as the basis for computing user credits, encouraging responsible usage and timely return of the power units.

\subsubsection{Rental Time Tracking and Rule-Based Control}
...
Using Flutter enabled the developers to create a single codebase that can be deployed across multiple platforms. The framework’s extensive widget library facilitated the development of an intuitive and responsive user interface for displaying system data such as battery level, rental status, notifications, and user credits. The hot reload feature also supported rapid development, testing, and refinement of the mobile application.
...
\subsubsection{Cloud Database and Backend Services}
Cloud-based services are utilized to support data storage, transmission, and system synchronization. These services enable secure handling of rental transactions, user records, and system logs.
...
\subsubsection{System Measurements and Calculations}
This section presents the specific measurements and calculations performed by the software to support rental management, power monitoring, and credit computation. These calculations are derived from sensor readings, system timestamps, and predefined rules integrated into the software logic.

\subsubsection{Battery Voltage Measurement}
The ESP32 continuously measures the battery voltage using its analog-to-digital converter (ADC). The measured ADC value is converted into the actual battery voltage using a voltage scaling factor based on the battery monitoring circuit.

This voltage value serves as the basis for determining battery percentage, power availability, and unused power.

\subsubsection{Battery Percentage Calculation}
Battery percentage is calculated by mapping the measured battery voltage to predetermined minimum and maximum voltage limits of the \ce{LiFePO4} battery.

\subsubsection{Rental Time Computation}
\subsubsection{Unused Power Calculation}
\subsubsection{Credit Computation}

\subsubsection{Location Measurement and Tracking}
The GPS module provides latitude and longitude values of the detachable power source. These coordinates are transmitted to the system and displayed in the mobile application to indicate the unit’s current location.

This location data supports monitoring, recovery, and security operations.

\subsubsection{Slot Availability and Lock Status Determination}
Slot availability is determined based on the presence of the detachable power source and rental state:
	
	\begin{itemize}
		\item {Free }\\
		\textit{Unit is available, Locked }

		\item {Occupied}\\
		\textit{Unit is currently rented, Unlocked}
	\end{itemize}

The lock status is controlled through software logic and depends on payment validation, and return confirmation.
